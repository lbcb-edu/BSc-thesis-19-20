...
def sam_file(x):
    if not x.lower().endswith('.sam'):
        raise argparse.ArgumentTypeError("'sam_file' argument needs to be SAM file.")
...
parser = argparse.ArgumentParser(
    description=
    """
    This script takes as input: reads which were mapped to transcript/already mapped reads and SAM alignment file. 
...
parser.add_argument(
    ...
    NOTE:
        Script will check if the file ends with '.sam' or any of its variations.
...
file_sam = open(args.sam_file, 'r', encoding='utf-8')


# Dictionaries:
#   -> current_split_dict_sam: (dictionary of type: read -> list of transcripts to which it mapped)
#   -> previous_split_dict_sam (dictionary of same type but made in previous split)
...
current_split_dict_sam = {}
previous_split_dict_sam = {}
mapping_to_transcripts = {}

line = file_sam.readline()
...
#    references in .sam file will be id's of previously mapped reads. To determine to which transcript bin
...
if which_split == '1st_split':
    while line != '':
        ...
        read_id = parts[0]
        mapped = parts[1]
        name_of_the_reference = parts[2]

        # We will read each line of SAM file and if the read mapped, add (key, value) pair to both
        # 'mapping_to_transcripts' and 'current_split_dict_sam'. In 'mapping_to_transcripts' keys are transcript names
        # and values lists of read_ids that mapped to the transcript. In 'current_split_dict_sam' keys are read_ids and
        ...
        if mapped == '0' and name_of_the_reference != '*':
            ...
            if current_split_dict_sam.get(read_id, 0) == 0:
                current_split_dict_sam[read_id] = [name_of_the_reference]
            else:
                current_split_dict_sam[read_id].append(name_of_the_reference)

        line = file_sam.readline()

    # saving dictionaries to pickle directory is the same in both cases so we will do it later on
    ...
    with open(args.pickle_directory + '/' + pickle_file_name, 'rb') as f:
        previous_split_dict_sam = pickle.load(f)
    ...
    while line != '':
        ...
        if mapped == '0' and name_of_the_reference != '*':
            ...
            transcripts_to_which_the_reference_mapped = previous_split_dict_sam[name_of_the_reference]
            ...
            for transcript in transcripts_to_which_the_reference_mapped:
                ...
                if current_split_dict_sam.get(read_id, 0) == 0:
                    current_split_dict_sam[read_id] = [transcript]
                else:
                    current_split_dict_sam[read_id].append(transcript)
...
with open(args.pickle_directory + '/' + which_split + '/read_mappings.p', 'wb') as f:
    pickle.dump(current_split_dict_sam, f)
...
if extension == '.fastq':
    ...
    while line1 != '':
        ...
        # if read's id is in dictionary 'current_split_dict_sam' that means it mapped
        if current_split_dict_sam.get(read_id, 0) != 0:
