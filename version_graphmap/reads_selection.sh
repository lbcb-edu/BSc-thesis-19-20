...
FA_tool="graphmap"
FA_PON="k6_A0.5"
FA_command='graphmap align --freq-percentile 1.0 --double-index --minimizer-window 1 -k 6 --ambiguity 0.5 --auto-rebuild-index -r $ref -d $reads -o ${output_dir}/reads_selection/1st_split/alignment/$FA_output || ( echo >&2 && echo "ERROR in reads_selection.sh script: 1st_split ALIGNMENT failed!" >&2 && echo >&2 && exit 1 ) || exit 1' #(will contain $reads $ref and $FA_output in it)

OA_tool="graphmap"
OA_PON="k6_A0.5"
OA_command='graphmap align --freq-percentile 1.0 --double-index --minimizer-window 1 -k 6 --ambiguity 0.5 --auto-rebuild-index -r ${output_dir}/reads_selection/${previous_split}/split_files/$OA_PRO_M -d ${output_dir}/reads_selection/${previous_split}/split_files/$OA_PRO_U -o ${output_dir}/reads_selection/$current_split/alignment/${OA_output} || ( echo >&2 && echo "ERROR in reads_selection.sh script: ${current_split} ALIGNMENT failed!" >&2 && echo >&2 && exit 1 ) || exit 1' #(will contain $OA_PRO_M $OA_PRO_U and $OA_output in it)
...
then
	...
	then
		...
		if [ -n "$FA_PON" ]
		then
			FA_output="${FA_tool}_alignment_${FA_PON}_${reads_name}_to_${ref_name}.sam"

		else
			FA_output="${FA_tool}_alignment_${reads_name}_to_${ref_name}.sam"
		fi
	...
	then
		...
		do
			...
			if [ -n "$OA_PON" ]
			then
				OA_output="${OA_tool}_alignment_${OA_PON}_${previous_split}_unmapped_to_mapped.sam"

			else
				OA_output="${OA_tool}_alignment_${previous_split}_unmapped_to_mapped.sam"
			fi
	...
	then
		...
		for file in ${output_dir}/reads_selection/*_split/split_files/*_only_mapped${extension}
		do
			cat $file
