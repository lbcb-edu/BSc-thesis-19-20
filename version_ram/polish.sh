...
FINAL_tool="graphmap"
FINAL_PON=""
FINAL_command='graphmap align -r $ref -d ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$polished_reads -o ${output_dir}/final_alignment/$FINAL_output || ( echo >&2 && echo "ERROR in polish.sh script: FINAL ALIGNMENT failed!" >&2 && echo >&2 && exit 1 ) || exit 1' #(will contain $polished_reads $ref and $FINAL_output in it)

OV_tool="ram"
OV_PON="k10_w4"

OV_command_1='../ram/build/bin/ram -k 10 -w 4 $reads_to_polish $reads_to_polish > ${output_dir}/polishing/1x/base_to_base_overlap/$OV_output || ( echo >&2 && echo "ERROR in polish.sh script: 1st OVERLAP failed!" >&2 && echo >&2 && exit 1 ) || exit 1' #(will contain $reads_to_polish and $OV_output in it)

OV_command_2='../ram/build/bin/ram -k 10 -w 4 ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$reads_to_polish ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$reads_to_polish > ${output_dir}/polishing/${polishing_round}x/$((polishing_round - 1))x_to_$((polishing_round - 1))x_overlap/$OV_output || ( echo >&2 && echo "ERROR in polish.sh script: $current_round OVERLAP failed!" >&2 && echo >&2 && exit 1 ) || exit 1'
...
then
	...
	then
		...
		if [ -n "$OV_PON" ]
		then
			OV_output="overlap_${OV_tool}_${OV_PON}_${RTP_FN}_to_${RTP_FN}.paf"

		else
			OV_output="overlap_${OV_tool}_${RTP_FN}_to_${RTP_FN}.paf"
		fi
		...
		../ram/build/bin/ram -k 10 -w 4 $reads_to_polish $reads_to_polish > ${output_dir}/polishing/1x/base_to_base_overlap/$OV_output || ( echo >&2 && echo "ERROR in polish.sh script: 1st OVERLAP failed!" >&2 && echo >&2 && exit 1 ) || exit 1
	...
	then
		...
		do
			...
			../ram/build/bin/ram -k 10 -w 4 ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$reads_to_polish ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$reads_to_polish > ${output_dir}/polishing/${polishing_round}x/$((polishing_round - 1))x_to_$((polishing_round - 1))x_overlap/$OV_output || ( echo >&2 && echo "ERROR in polish.sh script: $current_round OVERLAP failed!" >&2 && echo >&2 && exit 1 ) || exit 1
	...
	then
		...
		#graphmap align --freq-percentile 1.0 --double-index --minimizer-window 1 -k 6 --ambiguity 0.5 --auto-rebuild-index -r $ref -d ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$polished_reads -o ${output_dir}/final_alignment/$FINAL_output || ( echo >&2 && echo "ERROR in polish.sh script: FINAL ALIGNMENT failed!" >&2 && echo >&2 && exit 1 ) || exit 1
                graphmap align -r $ref -d ${output_dir}/polishing/$((polishing_round - 1))x/polished_files/$polished_reads -o ${output_dir}/final_alignment/$FINAL_output || ( echo >&2 && echo "ERROR in polish.sh script: FINAL ALIGNMENT failed!" >&2 && echo >&2 && exit 1 ) || exit 1
