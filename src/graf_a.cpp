...
namespace hera {
  	...
  	if (N > 0) {
	  	switch (pgType) {
	  		...
	  		case (PGT_MAXESRIGHT):
	  			...
	  			break;
	  		default:
	  			throw std::runtime_error(std::string("hera BRIDGER: ERROR - invalid path generation type!"));
	  	}
  	}
  ...
  int generatePathsDeterministic(vector<shared_ptr<Path>> &vPaths, MapIdToNode &aNodes, PathGenerationType pgType){
  	...
  	uint32_t numNodes = hera::NumDFSNodes;

  	if (hera::print_output)
  		std::cerr << "\nhera: Generating deterministic paths: ";
  	...
  	for (auto const& itANode : aNodes) {
  		...
  		for (auto const& edge_ptr : aNode->vOutEdges) {
            ...
            while (!eStack.empty()) {
                ...
                if ((uint32_t)(newPath->size()) >= hera::HardNodeLimit) continue;
                ...
                for (auto const& edge2_ptr : rnode_ptr->vOutEdges) {
                    ...
                    else throw std::runtime_error(std::string("hera BRIDGER: ERROR - invalid node type: ") + hera::NodeType2String(endNode->nType));
                }
  ...
  int generatePaths_MC(std::vector<shared_ptr<Path>> &vPaths, MapIdToNode &aNodes, uint32_t minNumPaths) {
  	 ...
  	 * It might be more efficient if Node pointers were used!
  	 */
  	uint32_t maxIterations = hera::MaxMCIterations;
